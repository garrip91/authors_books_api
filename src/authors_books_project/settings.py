...
DATABASES = {
    "default": {
        "ENGINE": os.getenv("DB_ENGINE"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА ВЫБОР ДВИЖКА БАЗЫ ДАННЫХ, КОТОРЫЙ БУДЕТ ИСПОЛЬЗОВАТЬСЯ ДЛЯ ВЗАИМОДЕЙСТВИЯ С БАЗОЙ ДАННЫХ
        "NAME": os.getenv("DB_NAME"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА УКАЗАНИЕ ИМЕНИ БАЗЫ ДАННЫХ, К КОТОРОЙ БУДЕТ ПОДКЛЮЧАТЬСЯ ПРОЕКТ
        "USER": os.getenv("DB_USER"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА УКАЗАНИЕ ИМЕНИ ПОЛЬЗОВАТЕЛЯ, КОТОРОЕ ИСПОЛЬЗУЕТСЯ ДЛЯ ПОДКЛЮЧЕНИЯ К БАЗЕ ДАННЫХ
        "PASSWORD": os.getenv("DB_PASSWORD"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА УКАЗАНИЕ ПАРОЛЯ, КОТОРЫЙ ИСПОЛЬЗУЕТСЯ ДЛЯ ПОДКЛЮЧЕНИЯ К БАЗЕ ДАННЫХ
        "HOST": os.getenv("DB_HOST"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА УКАЗАНИЕ АДРЕСА СЕРВЕРА БАЗЫ ДАННЫХ, К КОТОРОМУ БУДЕТ ПОДКЛЮЧАТЬСЯ ПРОЕКТ
        "PORT": os.getenv("DB_PORT"), # ЭТА НАСТРОЙКА ОТВЕЧАЕТ ЗА УКАЗАНИЕ ПОРТА, ЧЕРЕЗ КОТОРЫЙ ФРЕЙМВОРК БУДЕТ ПОДКЛЮЧАТЬСЯ К СЕРВЕРУ БАЗЫ ДАННЫХ
    }
}

# ПЕРЕКЛЮЧЕНИЕ НА ТЕСТОВУЮ БД (sqlite3) ДЛЯ ТЕСТИРОВАНИЯ:
