...
from authors_books_app import custom_permissions
...
class TestAuthorDetailView:
    ...
    def setup(self):
        ...
        self.url = reverse("author-detail", kwargs={"pk": self.author.pk}) # создаём ссылку для доступа к записи конкретного автора по ID

    # ПРОВЕРКА ВОЗМОЖНОСТИ ПОЛУЧЕНИЯ ДЕТАЛЕЙ КОНКРЕТНОЙ ЗАПИСИ ИЗ ТАБЛИЦЫ `Author` ПОДКЛЮЧЁННОЙ БД:
    ...
    # ПРОВЕРКА ВОЗМОЖНОСТИ ОБНОВЛЕНИЯ ДЕТАЛЕЙ КОНКРЕТНОЙ ЗАПИСИ ИЗ ТАБЛИЦЫ `Author` ПОДКЛЮЧЁННОЙ БД:
    def test_update_author_authenticated_owner(self):
        self.client.force_authenticate(user=self.user)
        data = {
            "first_name": "Мистер",
            "last_name": "Могущественный", # изменяем фамилию
            "date_of_birth": "1940-01-01",
            "date_of_death": "2020-01-01",
        }
        ...
        assert self.author.last_name == "Могущественный"
    ...
    def test_update_author_authenticated_not_owner(self):
        ...
        self.author.refresh_from_db()
        assert self.author.last_name == "Всемогущий" # данные не изменились
